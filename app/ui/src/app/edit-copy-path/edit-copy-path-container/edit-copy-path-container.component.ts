...
export class EditCopyPathContainerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getOneCopyPathHttpService.execute(parseInt(this.routeId)).subscribe((copyPath: CopyPathDto) => {
      this.copyPath = copyPath;
    });
  }
  onSubmitForm(copyPath: CopyPathDto) {
    this.editCopyPathHttpService.execute(parseInt(this.routeId), copyPath).subscribe(() => {
